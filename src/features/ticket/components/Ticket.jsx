...
const Ticket = ({ showtime, selectedSeats}) => {
  ...
  const handleContinue = () => {
    ...
    queryParams.set('showtime', JSON.stringify(showtime));
    queryParams.set('selectedSeats', JSON.stringify(selectedSeats));
  ...
  return (
    <div className={`ticket pb-9 bg-white ${isScrolled ? 'scrolled' : ''}`}>
      ...
      <div className="flex mb-3 items-center">
        ...
        <div className="w-1/2 text-base">{selectedSeats.map((seat) => seat.seat_number).join(', ')}</div>
      </div>
